...
struct ContiguousArrayParsingTests {
  private let testBuffer: ContiguousArray<UInt8> = [
  ...
  func parseRemainingBytes() throws {
    testBuffer.withParserSpan { span in
      let parsedArray = ContiguousArray(parsingRemainingBytes: &span)
  ...
  func parseArrayOfFixedSize() throws {
    ...
    try testBuffer.withParserSpan { span in
      let parsedArray = try ContiguousArray(parsing: &span, count: 5) { input in
      ...
      let parsedArray2 = try ContiguousArray(parsing: &span, count: 2) { input in
      ...
      #expect(throws: ParsingError.self) {
        _ = try ContiguousArray(parsing: &span, count: 1) { input in
          try UInt16(parsingBigEndian: &input)
        }
      }

      let lastByte = try ContiguousArray(
        parsing: &span,
        count: 1,
    ...
    try testBuffer.withParserSpan { span in
      let parsedArray1 = try ContiguousArray(parsing: &span, count: 0) { input in
      ...
      let parsedArray2 = try ContiguousArray(parsing: &span, count: 0) { input in
        try UInt64(parsingBigEndian: &input)
      }
  ...
  func parseArrayOfCustomTypes() throws {
    ...
    _ = [0x0f, 0xf0].withParserSpan { span in
      #expect(throws: TestError.self) {
        try ContiguousArray(parsingAll: &span, parser: CustomType.init(parsing:))
      }
    }
  }
  ...
  func parseAllAvailableElements() throws {
    // Parse as UInt8
    try testBuffer.withParserSpan { span in
      let parsedArray = try ContiguousArray(parsingAll: &span) { input in
        try UInt8(parsing: &input)
      }
      #expect(parsedArray == testBuffer)
      #expect(span.count == 0)
  ...
  func parseArrayWithErrorHandling() throws {
    ...
    try testBuffer.withParserSpan { span in
      ...
      #expect(throws: TestError.self) {
        _ = try ContiguousArray(parsing: &span, count: testBuffer.count) { input in
